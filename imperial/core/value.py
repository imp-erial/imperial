...
from ..exceptions import ImperialLibraryError, ImperialValueError
...
class Value(Dynamic):
	...
	def _register(cls):
		...
		class DataKey(Key):
			...
			@property
			def type(self) -> ImperialType:
				return type(self.container)

			def set(self, value: EitherValue):
				if self._py_data is None and self._data is None:
					if isinstance(value, ImperialType):
						self._py_data = None
						self.data = value
						self.defaulted = False
					else:
						value = self.container.normalize(value)
						# TODO: what to do when other keys are returned?
						assert not isinstance(value, dict)
						self._py_data = value
						self._data = self.container
						self.defaulted = False
				elif self._py_data is None:
					# Set thru proxy
					self.data.set(value)

			def imperialize(self, value: EitherValue) -> ImperialType:
				if isinstance(value, ImperialType):
					# Allow any value, for proxies. They'll do any complaining...
					return value
				return self.type(self.container.normalize(value))
	...
	def convert_to(self, type: Type[ImperialType]) -> ImperialType:
		...
		raise ImperialTypeError(
			f"no conversion from {self.__class__.__name__} to {type.__name__} known"
		)
